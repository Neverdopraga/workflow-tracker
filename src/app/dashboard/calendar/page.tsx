...
export default function CalendarPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (error) throw error; setTasks(data || []); setConnection("live");
      ...
      <div className="flex-1 p-4 sm:p-6 max-w-5xl mx-auto w-full space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                  ...
                  <button key={dateStr} onClick={() => setSelectedDate(dateStr === selectedDate ? null : dateStr)}
                    ...
                    {dayTasks.length > 0 && (
                      <div className="flex gap-0.5">
                        {dayTasks.slice(0, 3).map((t) => <span key={t.id} className={`w-1.5 h-1.5 rounded-full ${isSelected ? "bg-white/70" : statusDot[t.status]}`} />)}
                        {dayTasks.length > 3 && <span className={`text-[8px] font-bold ${isSelected ? "text-white/70" : "text-gray-400"}`}>+{dayTasks.length - 3}</span>}
                      </div>
                    )}
                  </button>
                );
          ...
          <div className="bg-white rounded-2xl border border-border p-5">
            ...
            ) : <p className="text-xs text-gray-400 text-center py-8">No tasks due</p>
            ) : <p className="text-xs text-gray-400 text-center py-8">Click a date to view tasks</p>}
          </div>
        </div>
      </div>
